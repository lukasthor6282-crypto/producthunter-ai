...
function storeActivePage(page: PageKey) {
  if (typeof window === "undefined" || !protectedPages.has(page)) return;
...
export function ProductHunterApp() {
  ...
  const { data, isLoading: isRecommendationLoading, error, rawError: rawRecommendationError, run, reset: resetRecommendation } = useRecommendations();
  const {
    authConfig,
    user,
    isAuthenticated,
  ...
  const recommendationQuotaError = useMemo(() => recommendationQuotaErrorFrom(rawRecommendationError), [rawRecommendationError]);
  ...
  const page = useMemo(() => {
    const pages: Record<PageKey, ReactNode> = {
      landing: <LandingPage onStart={startDemo} onNavigate={navigate} user={user} onLogin={() => navigate("login")} onLogout={handleLogout} isLoggingOut={isLoggingOut} />,
      login: (
        <LoginPage
    ...
    <div className={isPublicPage ? "min-h-screen bg-[#07090d] text-white" : "kombai-shell min-h-screen text-white"}>
      ...
      >
        ...
        <AnimatePresence mode="wait">
          <motion.div key={activePage} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }} className={isPublicPage ? "min-h-screen" : undefined}>
